        ...
        <View style={styles.group}>
          ...
          {facilities.map((facility, index) => (
            <Pressable
              ...
              style={styles.item}
              onPress={() => navigation.navigate('VenueDetail', { facilityId: Number(facility.fid ?? 0), title: facility.fname ?? 'Facility' })}
            >
              <Text style={styles.itemTitle}>{facility.fname ?? `Facility ${index + 1}`}</Text>
              <Text style={styles.itemMeta}>Open venue detail</Text>
            </Pressable>
          ))}
        </View>
