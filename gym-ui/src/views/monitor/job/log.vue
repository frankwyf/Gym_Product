<template>
  <div class="app-container">
    ...
    <!-- 调度日志详细 -->
    <el-dialog title="Scheduling log" :visible.sync="open" width="700px" append-to-body>
      <el-form ref="form" :model="form" label-width="100px" size="mini">
        <el-row>
          ...
          <el-col :span="24">
            <el-form-item label="Function: ">{{ form.invokeTarget }}</el-form-item>
          </el-col>
          <el-col :span="24">
          ...
          <el-col :span="24">
            <el-form-item label="Error: " v-if="form.status == 1">{{ form.exceptionInfo }}</el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('Confirm to delete schedule log entries with the number "' + jobLogIds + '"?').then(function() {
    ...
    /** 清空按钮操作 */
    handleClean() {
      this.$modal.confirm('Confirm to clear all schedule log entries?').then(function() {
        return cleanJobLog();
      }).then(() => {
        this.getList();
