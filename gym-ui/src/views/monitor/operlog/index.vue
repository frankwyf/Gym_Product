<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="Modules" prop="title">
        <el-input
    ...
    <el-table ref="tables" v-loading="loading" :data="list" @selection-change="handleSelectionChange" :default-sort="defaultSort" @sort-change="handleSortChange">
      ...
      <el-table-column label="Methods" align="center" prop="requestMethod" />
      ...
      <el-table-column label="Place" align="center" prop="operLocation" :show-overflow-tooltip="true" />
      <el-table-column label="Status" align="center" prop="status">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_common_status" :value="scope.row.status"/>
        </template>
      </el-table-column>
      <el-table-column label="Time" align="center" prop="operTime" sortable="custom" :sort-orders="['descending', 'ascending']" width="180">
        <template slot-scope="scope">
    ...
    <!-- 操作日志详细 -->
    <el-dialog title="Details of log" :visible.sync="open" width="700px" append-to-body>
      <el-form ref="form" :model="form" label-width="100px" size="mini">
        <el-row>
          ...
          <el-col :span="24">
            <el-form-item label="Return">{{ form.jsonResult }}</el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="Status">
              <div v-if="form.status === 0">Success</div>
              <div v-else-if="form.status === 1">Unsuccess</div>
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('Confirm to delete entries with log number "' + operIds + '"?').then(function() {
    ...
    /** 清空按钮操作 */
    handleClean() {
      this.$modal.confirm('Confirm to clear all operation log entries?').then(function() {
        return cleanOperlog();
      }).then(() => {
        this.getList();
        this.$modal.msgSuccess("Empty successfully");
      }).catch(() => {});
    },
    /** 导出按钮操作 */
    handleExport() {
